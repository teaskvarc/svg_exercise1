...
ctx.lineWidth = 4;          //kako debela bo crta s katero risemo objekte
ctx.lineCap = "round";      // robovi objekta so zaobljeni (lahko imamo se: 'butt', 'square')
...
function loop() {
    ctx.clearRect(0,0,200,200);  //The clearRect(x,y,width,height) method clears the specified pixels within a given rectangle
    offset = offset + 0.009;
...
function tweenPaths() {
   pathPointsFrom = paths[pathCount];
    ...
    TweenLite.to(interpolationPoint, 0.7, {percentage:1, ease: Power2.easeInOut,
        delay: 0.4, onComplete: function () {
